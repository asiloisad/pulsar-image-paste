...
export default class SaveDialog {
  ...
  constructor() {
    ...
    let blurHandler = () => { if (document.hasFocus()) { return this.hide() }}
    ...
    this.disposables.add(
      ...
      this.miniEditor.onDidChange( () => {
        this.overwrite = false ; this.warningElement.innerHTML = ''
      })
    )
  ...
  prepare(event, initialPath, editor) {
    ...
    let extName, md5, nm5, range, selection, selectionEnd, selectionStart, baseName
    ...
    if (range) {this.miniEditor.setSelectedBufferRange(range)};
    this.overwrite = false ; this.warningElement.innerHTML = ''
  ...
  confirm() {
    ...
    if (!this.overwrite && fs.existsSync(filePath)) {
      this.overwrite = true ; this.warningElement.innerHTML = 'WARNING: File already exists. Press <span class="keystroke">Enter</span> again to overwrite it.'
      return
    }
    ...
    if (this.editor) { this.editor.insertText(relPath) }
    this.hide()
  }
}
