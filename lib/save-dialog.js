...
export default class SaveDialog {
  ...
  constructor(initialPath, img, editor, basePath, range, dataURL) {
    ...
    this.warnText = document.createElement('div')
    this.warnText.classList.add('warning');
    this.element.appendChild(this.warnText)
  ...
  onConfirm(relPath) {
    ...
    if (!this.readyQ && fs.existsSync(filePath)) {
      this.warnText.innerHTML = 'WARNING: File already exists. Press <span class="keystroke">Enter</span> again to overwrite it.'
      this.readyQ = true
      return
    }
    extName = path.extname(filePath).toLowerCase()
    if (extName==='.jpg' || extName==='.jpeg') {
      imgbuffer = this.img.toJPEG(95);
    } else {
      imgbuffer = this.img.toPNG();
    }
