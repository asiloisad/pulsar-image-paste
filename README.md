...
3. Paste the data. Use `core:paste` if `Hack paste methods` is enabled, otherwise `image-paste:paste` command is required.
...
- Sometimes copied item may have multiple formats (e.g., tables from MS Excel). In such cases, you can paste as plain text to get the raw content.
